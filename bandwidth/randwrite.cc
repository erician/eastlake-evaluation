...
void Prepare(int pod, int thread_id, int all_threads_num) {
    ...
    // char *addr = (char *)mmap(NULL, EXTEND_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE | MAP_HUGETLB, -1, 0);
    if (addr == NULL || addr < 0) {
...
void RandWriteThread(int pod, int thread_id, int all_threads_num) {
    ...
    // }
    for (long i=0; i<BLOCK_NUM; i++) {
        NTwrite(addrs[thread_id]+access_patterns[thread_id][i]*BLOCK_SIZE, buff, BLOCK_SIZE);
    }
}
